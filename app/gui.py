...
class Gui(tk.Tk):
    def __init__(self, title: str):
        ...
        self._select_file_btn = self._create_button(row=1,column=1, text='Select file', command=self.select_file)
        ...
        self.combobox = self._create_combobox(row=0,column=1)
    ...
    def _create_button(self, row: int, column: int, text: str, command: Callable[[], None]) ->  ttk.Button:
        button = ttk.Button(self, text=text, command=command)
        button.grid(row=row,column=column, sticky='nsew')
        return button
    
    def _create_text_box(self, row: int, column: int, rowspan: int, columnspan: int) -> tk.Entry:
        text_box = tk.Entry(self)
        text_box.grid(row=row, column=column, rowspan=rowspan, columnspan=columnspan, sticky='nsew')
        return text_box
        
    def _create_combobox(self, row: int, column: int) -> ttk.Combobox:
        cipher_mode = ['ECB', 'CBC']
